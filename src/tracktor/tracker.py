...
class Tracker:
    ...
    def align(self, blob):
        ...
        if self.im_index > 0:
            ...
            for t in self.tracks:
                t.pos = warp_pos(t.pos, warp_matrix, is_cuda=self.use_gpu)
    ...
    def motion_step(self, track):
        ...
        if self.motion_model_cfg['center_only']:
            center_new = get_center(track.pos, is_cuda=self.use_gpu) + track.last_v
            track.pos = make_pos(*center_new, get_width(track.pos), get_height(track.pos), is_cuda=self.use_gpu)
        else:
    ...
    def motion(self):
        ...
        for t in self.tracks:
            ...
            if self.motion_model_cfg['center_only']:
                vs = [
                    get_center(p2, is_cuda=self.use_gpu) - get_center(p1, is_cuda=self.use_gpu)
